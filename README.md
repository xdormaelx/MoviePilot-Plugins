...
### 剧集字幕重命名（AutoSubRename）

自动识别监控目录中的字幕文件，根据字幕文件名与同目录视频文件中的季集信息（例如 `S01E01`）进行匹配，并将字幕重命名为对应视频文件的名称。

**主要功能：**

- 递归监控一个或多个目录及其子目录；
- 监听新建或移动的字幕文件，自动执行重命名；
- 支持一次性批量扫描并处理已有字幕文件；
- 支持剧集字幕按 `SxxExx` / `sxxexx` 季集编号匹配；
- 支持电影字幕：将字幕文件放入电影文件夹后，自动按同目录视频文件名重命名；
- 支持 `ass`、`ssa`、`srt`、`sup` 字幕格式；
- 支持自定义视频和字幕文件扩展名；
- 可选发送处理结果通知；
...
电影目录中如果有多个视频文件，插件只有在字幕名与其中一个视频名存在明确包含关系时才会重命名，避免误匹配。

**使用方式：**

1. 在 MoviePilot 插件市场安装并启用“剧集字幕重命名”；
2. 在插件配置中填写监控目录，每行一个目录；
3. 根据需要开启“发送通知”；
4. 点击“立即运行一次”处理已有字幕，或保持插件启用以持续监控新文件。

**匹配示例：**

```text
视频：某剧集.S01E01.1080p.mkv
字幕：某剧集.S01E01.ass
结果：某剧集.S01E01.1080p.ass
```

如果字幕文件中没有季集编号、同目录没有匹配的视频，或目标字幕文件已经存在，插件会跳过该文件并记录日志。
