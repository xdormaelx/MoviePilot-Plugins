...
from app.helper.notification import NotificationHelper  # 新增：使用通知帮助类
...
class CloudLinkMonitor(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        # 初始化通知服务
        self._notification_service = NotificationHelper()
        ...
        try:
            ...
            with lock:
                ...
                if not mediainfo:
                    ...
                    if self._notify:
                        # 使用通知服务发送消息
                        self._notification_service.post(
                            title=f"{file_path.name} 未识别到媒体信息",
                            text=f"无法入库！回复：```\n/redo {his.id} [tmdbid]|[类型]\n``` 手动识别转移。",
                            mtype=NotificationType.Plugin,
                            plugin_id=self.plugin_name,
                            channel=NotificationType.Manual
                        )
                    return
        ...
        for medis_title_year_season in list(self._medias.keys()):
            ...
            if (datetime.datetime.now() - last_update_time).total_seconds() > int(self._interval) \
                ...
                if self._notify:
                    ...
                    # 使用通知服务发送消息
                    self._notification_service.post(
                        title=f"【{self.plugin_name}】{mediainfo.title_year}{season_episode} 入库成功",
                        text=f"入库方式：{transferinfo.message}\n"
                             f"目标位置：{transferinfo.target_path}\n"
                             f"文件数量：{file_count}\n"
                             f"总大小：{StringUtils.str_filesize(total_size)}",
                        mtype=NotificationType.Plugin,
                        plugin_id=self.plugin_name,
                        image=mediainfo.get_message_image()
                    )
